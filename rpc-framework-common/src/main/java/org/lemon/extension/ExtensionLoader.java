package org.lemon.extension;

import lombok.extern.slf4j.Slf4j;
import org.lemon.utils.StringUtil;
...
public class ExtensionLoader<T> {
    ...
    public static <S> ExtensionLoader<S> getExtensionLoader(Class<S> type) {
        ...
        if (type.getAnnotation(SPI.class) == null) {
            throw new IllegalArgumentException("Extension type must be annotated as @SPI");
        }
    ...
    public T getExtension(String name) {
        ...
        // get from cache first, if not hit then create one
    ...
    private Map<String, Class<?>> getExtensionClasses() {
        // get the loaded extension classes from the cache
    ...
    private void loadResource(Map<String, Class<?>> extensionClasses, ClassLoader classLoader, URL resourceUrl) {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(resourceUrl.openStream(), StandardCharsets.UTF_8))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (ci >= 0) {
                    // ignore any char after '#' because they are comments
